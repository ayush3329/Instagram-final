...
import { HandleMessages,HandleZeroUneenMessage, MakeGroupAdmin, changeGroupName, createGroupChat, removeGroupAmdin } from './ws-controller/ws-controller'; 
...
websocket.on("request", (request) => {
    ...
    connections.on("message", (data:Message) => {
        ...
        } else if(query === 'make-group-admin'){
            MakeGroupAdmin(parsedData.packet as {fullname: string, gid: string}, UserToConnection)
        } else if(query === 'remove-admin'){
            removeGroupAmdin(parsedData.packet as {fullname: string, gid: string}, UserToConnection)
        }
