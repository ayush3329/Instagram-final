...
export type PacketType = chatDetail & { newChat?: boolean } |  ChatsType |  Group | zeroUnseenMessage | newGroupName | {fullname: string, gid: string}

// {fullname: string} contain name of that user whom we want to remove, block, make admin of group
...
export type receiveMessagePacket = {
    query: string;
    packet: createdGroupDetails | chatDetail | ChatsType | newGroupName | {success: boolean, newAdmin: string} | {fullname: string}
};
